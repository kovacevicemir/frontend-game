...
export const attackMonster = (player:Player, monster:Monster):IBattleResults =>{
    ...
    const addPlayerAttack = ():number =>{
        ...
        playerDamage = player.totalAttack - monster.attack;
        if(playerDamage >= 0){
    ...
    const addMonsterAttack = ():number =>{
        ...
        monsterDamage = monster.attack - player.totalAttack;
        if(monsterDamage >= 0){
    ...
    const computeOutcome = () =>{
        if(playerHP > 0){ //Win
            const drops = isDrop(monster.droppableItems, 75) as IItem[];
